...
export const ACTION_TYPES = {
  SEARCH_BUSINESSES: 'business/SEARCH_BUSINESSES',
...
export default (state: BusinessState = initialState, action): BusinessState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_BUSINESSES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_BUSINESSES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_BUSINESSES):
...
const apiSearchUrl = 'api/_search/businesses';

// Actions

export const getSearchEntities: ICrudSearchAction<IBusiness> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_BUSINESSES,
  payload: axios.get<IBusiness>(`${apiSearchUrl}?query=${query}`)
});
