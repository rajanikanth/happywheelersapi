...
import { getSearchEntities, getEntities } from './services.reducer';
...
export class Services extends React.Component<IServicesProps, IServicesState> {
  ...
  render() {
    const { servicesList, match } = this.props;
    return (
      <div>
              ...
              <tbody>
                {servicesList.map((services, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{services.type}</td>
                    <td>{services.driver ? <Link to={`driver/${services.driver.id}`}>{services.driver.id}</Link> : ''}</td>
